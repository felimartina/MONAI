...
TEST_RANDOM_SHAPES = [
    ...
    [
        {"spatial_size": 3, "max_spatial_size": 4, "random_center": True, "random_size": True},
        (1, 4, 5, 6),
        (1, 3, 4, 3),
    ],
]
...
class TestRandSpatialCrop(CropTest):
    ...
    def test_deprecated_roi_size(self):
        ...
        expected = RandSpatialCrop((2, 2), random_center=False)(data)
        with self.assertWarnsRegex(FutureWarning, "spatial_size"):
            result = RandSpatialCrop(roi_size=(2, 2), random_center=False)(data)
        assert_allclose(result, expected)

    def test_deprecated_max_roi_size(self):
        data = np.random.randint(0, 2, (1, 4, 5, 6))
        expected = RandSpatialCrop(
            spatial_size=3, max_spatial_size=4, random_center=True, random_size=True
        )
        expected.set_random_state(seed=123)
        expected_out = expected(data)
        with self.assertWarnsRegex(FutureWarning, "max_spatial_size"):
            cropper = RandSpatialCrop(spatial_size=3, max_roi_size=4, random_center=True, random_size=True)
        cropper.set_random_state(seed=123)
        assert_allclose(cropper(data), expected_out)
